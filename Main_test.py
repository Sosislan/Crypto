...
# Основна стратегія
def trade_with_targets(symbol, leverage=20, atr_multiplier=1.7):
    df_15m = get_data(symbol, '15m')
    df_1d = get_data(symbol, '1d')
    ...
    avg_price_change = np.mean(np.diff(close_prices_15m[-14:]))
    ...
    if atr < low_volatility and not strong_trend:
        multipliers = [1.5, 2, 2.2]  # Слабкий тренд + низька волатильність (агресивні цілі)
        ...
        multipliers = [1.2, 1.5, 2]  # Сильний тренд + низька волатильність
    else:
        multipliers = [1, 1.5, 2.1]  # Стандартний варіант
    result = analyze_news_impact(symbol)

    # Визначення сигналів для покупки або продажу
    if (trend == "bullish" and rsi < 50 and macd > signal_line and adx > 18 and stoch < 22 and cci < -90 and
    ...
    elif (trend == "bearish" and rsi > 50 and macd < signal_line  and adx > 18 and stoch > 78 and cci > 90 and
          close_prices_15m.iloc[-1] < ema_15m * 1 and close_prices_15m.iloc[-1] < sar * 1 and (
    ...
    # Оцінка часу досягнення цілей
    estimated_times = [estimate_time_to_target(close_prices_15m.iloc[-1], t, avg_price_change) for t in targets]
    ...
    start_message = bot.send_message(
        ...
        text=(f"ВНИМАНИЕ СТАРТ СДЕЛКИ\n"
              ...
              f"Плечи: x{leverage}\n"
              f"РМ: 5.0%\n"
              f"Текущая цена: {close_prices_15m.iloc[-1]}\n"
              f"Цена входа {close_prices_15m.iloc[-1]}\n"
              f"Цель 1: {targets[0]} \n Час досягнення: {estimated_times[0]}\n"
              f"Цель 2: {targets[1]} \n Час досягнення: {estimated_times[1]}\n"
              f"Цель 3: {targets[2]} \n Час досягнення: {estimated_times[2]}\n"
              f"SL: {stop_loss}")
    )
